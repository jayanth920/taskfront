...
const fetcher = (url: string) =>
  api.get(url).then((r) =>
    r.data.map((t: any) => ({ ...t, id: t._id })) // map _id -> id
  );
...
export default function Board() {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      ...
      if (msg.type === "init") mutate(msg.tasks, false);
      else if (msg.type === "task_created")
        mutate((tasks: Task[] = []) => [...tasks, msg.task], false);
      else if (msg.type === "task_updated")
        mutate(
          (tasks: Task[] = []) =>
            tasks.map((t: Task) => (t.id === msg.task.id ? msg.task : t)),
          false
        );
      else if (msg.type === "task_deleted")
        mutate((tasks: Task[] = []) => tasks.filter((t: Task) => t.id !== msg.id), false);
      else if (msg.type === "tasks_reorder") mutate(msg.tasks, false);
    };
    return () => ws.close();
  }, []);
  ...
  function onDragEnd(result: DropResult) {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
      ws.send(JSON.stringify({ type: "reorder", tasks: newTasks }));
    } else {
      api.put(`/tasks/${moved.id}`, { column: destCol }).catch(console.error);
    }
  }
...
async function editTask(id: string) {
  const newT = prompt("Edit task title");
  if (!newT) return;
  await api.put(`/tasks/${id}`, { title: newT });
}
            ...
            <div key={col.id} className="bg-white p-3 rounded shadow-sm">
              ...
              <Droppable droppableId={col.id}>
                {(provided) => (
                  ...
                  >
                    {(grouped[col.id] || []).map((task: Task, index: number) => (
                          ...
                          >
                            <div className="flex justify-between items-start gap-2">
                              ...
                              <div className="flex gap-1">
                                <button className="text-xs px-2 py-1 rounded border text-blue-600" onClick={() => editTask(task.id)}>Edit</button>
                                <button className="text-xs px-2 py-1 rounded border text-red-600" onClick={() => deleteTask(task.id)}>Del</button>
                              </div>
                            </div>
                          </div>
                        )}
